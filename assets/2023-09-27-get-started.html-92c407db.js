import{_ as s}from"./plugin-vue_export-helper-c27b6911.js";import{r,o,c as l,a as e,b as a,d as t,f as n}from"./app-81fad4d6.js";const d={},c=n(`<div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>lang: zh-CN
title: Prometheus环境搭建
headerDepth: 0
description: Prometheus环境搭建
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>本文是Prometheus环境的搭建和安装，包括prometheus-sever、node-exporter、grafna的安装。安装成功后，配置prometheus采集node-exporter的指标，然后在grafana展示node-exporter采集的指标。</p><h2 id="安装prometheus" tabindex="-1"><a class="header-anchor" href="#安装prometheus" aria-hidden="true">#</a> 安装prometheus</h2>`,3),u={href:"https://prometheus.io/download/",target:"_blank",rel:"noopener noreferrer"},m=n(`<figure><img src="https://static.javajike.com/img/2023/10/8/image-20231008083006727.png" alt="image-20231008083006727" tabindex="0" loading="lazy"><figcaption>image-20231008083006727</figcaption></figure><p>下载完成后，执行解压：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>tar -zxvf prometheus-2.47.1.darwin-amd64.tar.gz 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>具体的配置介绍如下：</p>`,5),v=e("li",null,"global.scrape_interval 是全局配置默认采集时间间隔（周期性采集监控目标）",-1),h=e("li",null,"global.evaluation_interval是全局配置告警规则任务的计算",-1),_=e("li",null,"alerting.alertmanagers是配置alertmanagers的地址的，alertmanagers是用来处理prometheus根据告警规则任务计算出来的告警消息的",-1),b=e("li",null,"rule_files是配置告警规则文件的路径，可以使用通配符",-1),f={href:"http://localhost:9090/metrics%EF%BC%8C%E5%8F%AF%E4%BB%A5%E5%BE%97%E5%88%B0prometheus%E7%9A%84%E7%9B%91%E6%8E%A7%E6%8C%87%E6%A0%87%EF%BC%8C%E5%86%85%E5%AE%B9%E5%A6%82%E4%B8%8B%EF%BC%9A",target:"_blank",rel:"noopener noreferrer"},E=n(`<div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># HELP go_gc_duration_seconds A summary of the pause duration of garbage collection cycles.
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,4),x={href:"http://localhost:9100/metrics%EF%BC%8C%E5%B0%B1%E5%8F%AF%E4%BB%A5%E6%9F%A5%E7%9C%8Bnode_exporter%E6%9A%B4%E9%9C%B2%E7%9A%84%E7%AB%AF%E5%8F%A3%E6%8C%87%E6%A0%87%E3%80%82",target:"_blank",rel:"noopener noreferrer"},B=n(`<p>在prometheus的配置文prometheus.yml中，增加采集Node Exporter的任务，配置信息如下：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>scrape_configs:
  - job_name: &quot;prometheus&quot;
    static_configs:
      - targets: [&quot;localhost:9090&quot;]
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>grafana有很多配置项在conf目录下default.ini文件下，此篇文章不讲解这部分内容。</p><p>启动成功后，进入grafana的页面，地址为localhost:3030 ，grafana的默认登陆用户名admin、密码为admin</p><p>登陆成功后，需要为grafana配置prometheus的数据源，配置界面的路径和配置如下：</p><p><img src="https://static.javajike.com/img/2023/10/9/image-20231009220718330.png" alt="image-20231009220718330" loading="lazy">配置完成后保存。</p>`,17),q={href:"https://grafana.com/grafana/dashboards/?pg=community&plcmt=topnav&search=node+exporter+mac",target:"_blank",rel:"noopener noreferrer"},C=n('<p>搜索成功后，在本地部署的grafana页面导入node exporter的dashboard，页面的导入路径如下：</p><figure><img src="https://static.javajike.com/img/2023/10/10/image-20231010201727866.png" alt="image-20231010201727866" tabindex="0" loading="lazy"><figcaption>image-20231010201727866</figcaption></figure><p>导入成功后，就可以看到本机的资源dashboard（由node exporter采集的指标生产）</p><figure><img src="https://static.javajike.com/img/2023/10/9/image-20231009220040376.png" alt="image-20231009220040376" tabindex="0" loading="lazy"><figcaption>image-20231009220040376</figcaption></figure><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><ul><li>prometheus作为一个采集组件，采集了node-exporter的指标</li><li>同时prometheus作为一个时序数据库，存储了node-expoter的指标时序</li><li>grafana作为数据展示前端，读取prometheus的指标时序，并展示在监控大盘上。</li></ul>',6);function k(F,P){const i=r("ExternalLinkIcon");return o(),l("div",null,[c,e("p",null,[a("去prometheus官网下载prometheus，下载地址为"),e("a",u,[a("https://prometheus.io/download/"),t(i)]),a(" ，目前提供了Linux\\windows\\mac的版本，笔者这里下载mac版本的。")]),m,e("p",null,[a("启动成功后，访问"),e("a",g,[a("http://localhost:9090"),t(i)]),a(" ，就可以访问prometheus页面了，这个页面提供了对prometheus数据的查询和告警信息的查询，页面展示如下：")]),p,e("ul",null,[v,h,_,b,e("li",null,[a("scrape_configs是配置采集任务的，支持多种类型的采集，比如从k8s、consul、也可以配置静态采集任务。上面的配置是采集prometheus自身的监控指标。可以访问prometheus自身的监控指标的暴露端点："),e("a",f,[a("http://localhost:9090/metrics，可以得到prometheus的监控指标，内容如下："),t(i)])])]),E,e("p",null,[a("下载完成后，启动node_exporter，node_exporter的端口为9100，访问"),e("a",x,[a("http://localhost:9100/metrics，就可以查看node_exporter暴露的端口指标。"),t(i)])]),B,e("p",null,[a("访问prometheus的网页，"),e("a",A,[a("http://localhost:9090/graph，在查询框里输入node_filesystem_free_bytes（文件系统里剩余的磁盘字节数），就可以看到各个目录的磁盘剩余字节数了。"),t(i)])]),y,e("p",null,[a("然后去grafana官方网站的dashboard社区找一个node exporter的面板，搜索地址："),e("a",q,[a("https://grafana.com/grafana/dashboards/?pg=community&plcmt=topnav&search=node+exporter+mac"),t(i)])]),C])}const z=s(d,[["render",k],["__file","2023-09-27-get-started.html.vue"]]);export{z as default};
